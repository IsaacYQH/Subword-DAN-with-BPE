...
def read_sentiment_examples(infile: str) -> List[SentimentExample]:
    ...
    :cleaned_words: words from the original sentences removing empty string
    """
    f = open(infile)
    exs = []
    for line in f:
        if len(line.strip()) > 0:
            ...
            if len(fields) != 2:
                ...
                label = 0 if "0" in fields[0] else 1
                sent = fields[1].lower()
            cleaned_words = list(filter(lambda x: x != '', sent.rstrip().split(" ")))
            exs.append(SentimentExample(cleaned_words, label))
    f.close()
    return exs
